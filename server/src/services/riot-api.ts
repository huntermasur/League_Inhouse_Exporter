...
async function loadChampionCache(): Promise<void> {
  if (Date.now() - cacheLoadedAt < CACHE_TTL_MS && Object.keys(championIdMap).length > 0) {
    return;
  }
  ...
  const champRes = await fetch(`${CHAMPIONS_URL_BASE}/cdn/${latest}/data/en_US/champion.json`);
...
function getApiKey(): string {
  const row = db.prepare('SELECT value FROM settings WHERE key = ?').get('riot_api_key') as
    | { value: string }
    | undefined;
...
export async function fetchMatch(numericId: string): Promise<import('../types/api.js').RiotMatchResponse> {
  ...
  if (res.status === 401 || res.status === 403) {
    throw new Error('Invalid or expired API key. Please update it in Settings.');
  }
  if (res.status === 404) {
    throw new Error(`Match ${numericId} not found. Check the game ID and try again.`);
  }
  if (res.status === 429) {
    throw new Error('Rate limited by Riot API. Please wait a moment and try again.');
  }
  if (!res.ok) {

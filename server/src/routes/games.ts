...
router.get("/:matchId", (req: Request, res: Response) => {
  ...
  const players = db
    .prepare(
      "SELECT * FROM player_entries WHERE match_id = ? ORDER BY team, id",
    )
    .all(matchId) as unknown as StoredPlayer[];

  const bans = db
    .prepare(
      "SELECT * FROM ban_entries WHERE match_id = ? ORDER BY team, pick_turn",
    )
...
router.post("/", async (req: Request, res: Response) => {
  ...
  if (!game_id || !/^\d+$/.test(game_id.trim())) {
    res
      .status(400)
      .json({ error: "game_id must be a numeric match ID (e.g. 5515325815)" });
    return;
  }
  ...
  const existing = db
    .prepare("SELECT match_id FROM games WHERE match_id = ?")
    .get(matchId);
  if (existing) {
    res
      .status(409)
      .json({ error: `Game ${numericId} is already in the database.` });
    return;
  }

  ...
  return (
    <div className={styles.team}>
      <div className={styles.teamHeader}>
        <span className={`${styles.teamLabel} ${teamId === 100 ? styles.blue : styles.red}`}>
          {label}
        </span>
        ...
        <tbody>
          {players.map((p) => (
            <tr key={p.id} className={styles.row}>
              ...
              <td className={`${styles.td} ${styles.champion}`}>{p.champion}</td>
...
export function GameDetail({ matchId }: Props) {
  ...
  if (!matchId) {
    return <p className={styles.placeholder}>Select a game from the list to view details.</p>;
  }
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        <h2 className={styles.matchId}>Game <span className={styles.mono}>#{numericId}</span></h2>
        <span className={styles.info}>{dateStr} · {durationMins}m {durationSecs}s</span>
      </div>

      <TeamSection teamId={100} players={blueTeam} bans={blueBans} won={blueWon} />
      <TeamSection teamId={200} players={redTeam} bans={redBans} won={!blueWon} />
    </div>
  );
}
